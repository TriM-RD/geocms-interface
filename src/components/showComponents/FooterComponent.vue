<template>
  <nav v-if="renderComponent" class="navbar fixed-bottom navbar-light bg-light">
    <form class="container-fluid justify-content-around">
      <component
          v-for="(_objectTemplate, key, index) in objectTemplates"
          :key="`${key}-${index}`"
          :is="getComponent(_objectTemplate.Region, _objectTemplate.ObjectEnum)"
...
export default class FooterComponent extends Vue {
  ...
  mounted() {
    if (router.currentRoute.value.name !== undefined && router.currentRoute.value.name !== null) {
      this.openTab = router.currentRoute.value.name;
    }
    this.choose();
  }
  ...
  choose() {
    ...
    switch (this.openTab) {
      ...
      case Definitions.Permission.Def:
        this.objectTemplates = this.mechanic.InitSet(this.scaffoldButtons());
        break;
      ...
      case Definitions.Entity.Edit:
        this.objectTemplates = this.mechanic.InitSet([
          new ObjectTemplate(
              RegionEnum.Form,
              ObjectTypeEnum.Button,
              SubObjectTypeEnum.Left,
              ActionTypeEnum.Click,
              {
                [StatTypeEnum.Label]: StatType.StatTypes[StatTypeEnum.Label]()
                    .CreateStat()
                    .InitData($t.save),
                [StatTypeEnum.Design]: StatType.StatTypes[StatTypeEnum.Design]()
                    .CreateStat()
                    .InitData('btn btn-outline-success me-2 flex-fill'),
                [StatTypeEnum.Disabled]: StatType.StatTypes[StatTypeEnum.Disabled]()
                    .CreateStat()
                    .InitData('')
              }
          ),
          new ObjectTemplate(
              ...
              ActionTypeEnum.Click,
              {
                ...
                [StatTypeEnum.Design]: StatType.StatTypes[StatTypeEnum.Design]()
                    .CreateStat()
                    .InitData('btn btn-outline-danger flex-fill'),
          ...
          new ObjectTemplate(
              ...
              ActionTypeEnum.Click,
              {
                ...
                [StatTypeEnum.Design]: StatType.StatTypes[StatTypeEnum.Design]()
                    .CreateStat()
                    .InitData('btn btn-outline-secondary flex-fill')
              }
          )
        ]);
        break;
  ...
  scaffoldButtons(
      leftRegion = RegionEnum.Form,
      rightRegion = RegionEnum.Form,
      leftButtonName = $t.save,
      rightButtonName = $t.cancel
  ): Array<any> {
    return [
      new ObjectTemplate(
          leftRegion,
