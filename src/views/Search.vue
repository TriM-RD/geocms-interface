<template>
  <SearchComponentDemo v-if="renderComponent"></SearchComponentDemo>
</template>

<script lang="ts">
import { Component, Vue } from 'vue-facing-decorator'
import SearchComponentDemo from '@/components/SearchComponentDemo.vue'
...
export default class Search extends Vue {
  ...
  async created() {
    await fetch(`https://campsabout.com/mapAPI/test.php?id=${this.mapSettingsStore.get('propertyId')}&group=${this.mapSettingsStore.get('groupName')}`).then(async (response) => {
      ...
      this.classColorsStore.set(Object.assign(this.classColors, result))
    })
    await fetch(`https://campsabout.com/mapAPI/gjobjekti.php?id=${this.mapSettingsStore.get('propertyId')}&group=${this.mapSettingsStore.get('groupName')}`).then(async (response) => {
      if (!response.ok) return
      const data = await response.json()
      this.mapObjectsStore.set(data)
    })
